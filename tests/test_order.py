...
from constants import constructor_locators as loc, feed_locators as f_loc
...
class TestConstructorPage:
    ...
    def test_follow_constructor(self):
        ...
        assert order_page.driver.current_url == f'{urls.MAIN_URL}/', 'переход к Конструктору заказов не произошел'
    ...
    def test_follow_feed(self):
        ...
        order_page.wait_visibility(10, f_loc.DONE_ALL)
        assert order_page.driver.current_url == urls.EP_ORDER_FEED, 'переход к к Ленте заказов не произошел'
    ...
    def test_follow_ingredient_popup(self):
        ...
        assert order_page.get_text(loc.INGREDIENT_NAME) == order_page.get_text(loc.INGREDIENT_NAME_IN_POPUP), \
            'название ингредиента в конструкторе не соответствует названию в поп-апе'
    ...
    def test_ingredient_counter(self):
        ...
        assert before_adding < after_adding, 'счётчик ингредиента не увеличился'
    ...
    def test_make_order_by_user(self, auth):
        ...
        assert order_page.driver.find_element(*loc.CONFIRMATION_POPUP), 'поп-ап заказа не появился'
